...
# Use positional indexing (col names vary with BOM)
names(maine_ref)[c(1, 3, 16, 17, 18)] <- c("county", "dist", "rcv_yes", "rcv_no", "rcv_blank")
maine_ref <- maine_ref %>%
  filter(county != "", county != "CTY") %>%
  select(county, dist, rcv_yes, rcv_no, rcv_blank) %>%
  filter(dist != "MUNICIPALITY") %>%
  mutate(across(rcv_yes:rcv_blank, parse_number)) %>%
  mutate(rcv_share = rcv_yes / (rcv_yes + rcv_no))

names(maine_pres)[1:2] <- c("county", "dist")
maine_pres <- maine_pres %>%
  filter(county != "", county != "CTY") %>%
  select(county, dist, clinton = Clinton..Hillary.R., total = TBC) %>%
  filter(dist != "Town") %>%
...
```{r other-cities}
# These use left_join which keeps all LHS rows — just check for NAs
